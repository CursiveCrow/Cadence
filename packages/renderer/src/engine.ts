...
export class TimelineRendererEngine {
    ...
    render(data: { tasks: EngineTasks; dependencies: EngineDependencies; staffs: any[]; selection: string[] }, viewport: { x: number; y: number; zoom: number }): void {
        ...
        ensureGridAndStaff(layers.background, this.opts.config, data.staffs as any, this.opts.utils.getProjectStartDate(), app.screen.width, app.screen.height, currentZoom)
        ...
        for (const task of Object.values(data.tasks)) {
            const layout = computeTaskLayout(cfg, task as any, projectStartDate, data.staffs as any)
            const { container } = scene.upsertTask(task as any, layout, cfg, (task as any).title, (task as any).status)
        ...
        for (const id of data.selection) scene.drawSelection(id, cfg)
    }
