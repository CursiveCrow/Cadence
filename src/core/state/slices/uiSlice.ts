...
const uiSlice = createSlice({
    ...
    reducers: {
        ...
        setSidebarWidth: (state, action: PayloadAction<number>) => {
            const w = Math.round(action.payload)
            state.sidebarWidth = Math.max(180, Math.min(320, Number.isFinite(w) ? w : 220))
        },
    },
})

export const { setSelection, setSelectionWithAnchor, setViewport, setVerticalScale, setSidebarWidth } = uiSlice.actions
