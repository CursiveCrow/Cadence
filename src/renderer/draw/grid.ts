...
export function drawGridBackground(params: {
    ...
    for (let day = startDay; day <= endDay; day++) {
        ...
        if (isBarLine) {
            ...
            grid.stroke({ width: 2, color: 0x4a5568, alpha: 0.3 })

            // Accent line
            grid.moveTo(xl + 1.5, 0)
            grid.lineTo(xl + 1.5, height)
            grid.stroke({ width: 1, color: 0x4a5568, alpha: 0.2 })
        } else {
            // Regular beat lines
            grid.moveTo(xl, 0)
            grid.lineTo(xl, height)
            grid.stroke({ width: 1, color: 0x1e293b, alpha: 0.6 })
        }

        // Sub-beat dots (like staccato marks)
        if (pxPerDay > 40 && !isBarLine) {
            for (let i = 0.25; i < 1; i += 0.25) {
                const subX = xInt + i * pxPerDay
                if (subX < width) {
                    for (let y = 50; y < height; y += 100) {
                        grid.circle(subX, y, 0.5)
                        grid.fill({ color: 0xa855f7, alpha: 0.15 })
                    }
                }
            }
        }
...
export function drawStaffLines(params: {
    ...
    s.stroke({ width: 2, color: 0xa855f7, alpha: 0.2 })

    // Clef dot
    s.circle(clefX, clefCenterY, 3)
    s.fill({ color: 0xa855f7, alpha: 0.3 })
    ...
    for (let i = 0; i < lines; i++) {
        ...
        s.stroke({ width: lineWidth, color: 0x4a5568, alpha: lineAlpha })
