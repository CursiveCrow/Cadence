...
export function worldDaysToScreenX(
  ...
  leftMargin: number = TIMELINE.LEFT_MARGIN,
...
export function safeDiv(numerator: number, denominator: number, eps: number = EPS): number {
  return numerator / Math.max(denominator, eps)
}
