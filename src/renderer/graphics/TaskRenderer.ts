...
import { dayIndexFromISO } from '../utils'
...
export class TaskRenderer {
    ...
    private pruneUnusedCache(visibleTaskIds: string[]) {
        try {
            for (const [taskId, nodes] of this.taskCache) {
                if (!visibleTaskIds.includes(taskId)) {
                    try { nodes.head.destroy() } catch (err) {
                        if (import.meta?.env?.DEV) console.debug('[TaskRenderer]destroy head', err)
                    }
                    try { nodes.labelBox.destroy() } catch (err) {
                        if (import.meta?.env?.DEV) console.debug('[TaskRenderer]destroy labelBox', err)
                    }
                    try { nodes.labelMast.destroy() } catch (err) {
                        if (import.meta?.env?.DEV) console.debug('[TaskRenderer]destroy labelMast', err)
                    }
                    try { (nodes.labelText as any).destroy?.() } catch (err) {
                        if (import.meta?.env?.DEV) console.debug('[TaskRenderer]destroy labelText', err)
                    }
                    if (nodes.glyph) {
                        try { (nodes.glyph as any).destroy?.() } catch (err) {
                            if (import.meta?.env?.DEV) console.debug('[TaskRenderer]destroy glyph', err)
                        }
                    }
            ...
            if (import.meta?.env?.DEV) console.debug('[TaskRenderer]prune cache', err)
        }
    }
