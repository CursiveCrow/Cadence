...
import { Container, Graphics, Application, Text, Rectangle } from 'pixi.js'
...
export function drawGridAndStaff(
  ...
  staffs.forEach((staff) => {
    ...
    if ((config as any).DRAW_STAFF_LABELS !== false) {
      const labelText = new Text(staff.name, {
      ...
      const clefText = new Text(clefSymbol, {
        fontFamily: 'serif',
        fontSize: 20,
        fontWeight: 'bold',
        fill: 0xffffff
...
export class TimelineSceneManager {
  ...
  updateHoverAtViewportX(px: number | null, config: TimelineConfig, screenHeight: number): void {
    ...
    const t = this.hoverText || new Text(label, { fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', fontSize: 11, fill: 0xffffff })
