...
export class WebPlatformServices implements PlatformServices {
  async showOpenDialog(options: FileDialogOptions): Promise<FileHandle | null> {
    return new Promise((resolve) => {
      ...
      input.type = 'file'; input.multiple = false
      ...
      input.onchange = async (e) => {
        ...
        if (file) { const content = await file.arrayBuffer(); resolve({ name: file.name, path: file.name, content }) }
        else resolve(null)
      }
  ...
  async readFile(_path: string): Promise<ArrayBuffer> { throw new Error('Reading arbitrary file paths not supported in web environment') }
  async writeFile(path: string, content: ArrayBuffer): Promise<void> {
    try {
      const w = window as any
      if (w.showSaveFilePicker) {
        const handle = await w.showSaveFilePicker({ suggestedName: path })
        const writable = await handle.createWritable(); await writable.write(content); await writable.close(); return
      }
    } catch {}
    const blob = new Blob([content]); const url = URL.createObjectURL(blob)
    const a = document.createElement('a'); a.href = url; a.download = path; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url)
  }
  async showMessageBox(options: MessageBoxOptions): Promise<MessageBoxResult> {
    if (options.type === 'question' && options.buttons && options.buttons.length === 2) {
      const result = confirm(`${options.title}\n\n${options.message}`); return { response: result ? 0 : 1 }
    } else { alert(`${options.title}\n\n${options.message}`); return { response: 0 } }
  }
  async showErrorDialog(title: string, message: string): Promise<void> { alert(`${title}\n\n${message}`) }
  async getAppVersion(): Promise<string> { return 'web-version' }
