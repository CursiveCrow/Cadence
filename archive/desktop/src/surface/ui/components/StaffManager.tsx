...
export interface StaffManagerProps { isOpen: boolean; staffs: Staff[]; onClose: () => void; onAdd: (name: string, lines: number) => void; onUpdate: (id: string, updates: Partial<Staff>) => void; onDelete: (id: string) => void; onMoveUp: (id: string, index: number) => void; onMoveDown: (id: string, index: number) => void }
          ...
          {staffs.map((staff, index) => (
            <div key={staff.id} className="ui-rounded-md" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, marginBottom: 8, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <div className="ui-flex ui-gap-1 ui-items-center">
                ...
                <button onClick={() => setEditingStaff(editingStaff === staff.id ? null : staff.id)} className="ui-btn ui-rounded-sm" style={{ width: 28, height: 28 }}>✎</button>
                <button onClick={() => onDelete(staff.id)} disabled={staffs.length <= 1} className="ui-btn ui-rounded-sm" style={{ width: 28, height: 28, cursor: staffs.length <= 1 ? 'not-allowed' : 'pointer' }}>🗑</button>
              </div>
            </div>
          ))}
        </div>
