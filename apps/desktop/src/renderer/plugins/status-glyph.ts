...
export const StatusGlyphPlugin: RendererPlugin = {
  onTaskUpserted: (_task, container: Container, ctx) => {
    try {
      ...
      const t = new Text({ text: accidental, style: { fontFamily: 'serif', fontSize: baseFont * oversample, fontWeight: 'bold', fill: 0xffffff, align: 'center' } })
      const viewport = container.parent?.parent as Container | undefined
      const scaleX = viewport?.scale?.x ?? 1
      const desiredRes = computeTextResolution(scaleX, oversample); (t as any).resolution = desiredRes
      ...
      container.addChild(t); (container as any).__statusGlyph = t
