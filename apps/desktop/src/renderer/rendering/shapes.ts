...
export function drawNoteBodyPathAbsolute(graphics: Graphics, x: number, topY: number, width: number, height: number): void {
...
export function drawDependencyArrow(graphics: Graphics, srcX: number, srcY: number, dstX: number, dstY: number, color: number): void {
  graphics.clear()
  graphics.moveTo(srcX, srcY)
  const controlOffset = Math.abs(dstX - srcX) * 0.3
  graphics.bezierCurveTo(srcX + controlOffset, srcY, dstX - controlOffset, dstY, dstX, dstY)
  graphics.stroke({ width: 2, color, alpha: 0.6 })
  const angle = Math.atan2(dstY - srcY, dstX - srcX)
  const arrowSize = 8
  graphics.beginPath()
  graphics.moveTo(dstX, dstY)
  graphics.lineTo(dstX - arrowSize * Math.cos(angle - Math.PI / 6), dstY - arrowSize * Math.sin(angle - Math.PI / 6))
  graphics.lineTo(dstX - arrowSize * Math.cos(angle + Math.PI / 6), dstY - arrowSize * Math.sin(angle + Math.PI / 6))
  graphics.closePath()
  graphics.fill({ color, alpha: 0.6 })
}

export function drawSelectionHighlight(container: Container, config: TimelineConfig, layout: TaskLayout): Graphics {
  ...
  selectionGraphics.quadraticCurveTo(layout.startX + layout.width + selectionPadding, layout.topY - selectionPadding, layout.startX + layout.width + selectionPadding, layout.topY + 4)
  selectionGraphics.lineTo(layout.startX + layout.width + selectionPadding, layout.topY + config.TASK_HEIGHT - 4)
  selectionGraphics.quadraticCurveTo(layout.startX + layout.width + selectionPadding, layout.topY + config.TASK_HEIGHT + selectionPadding, layout.startX + layout.width - 4, layout.topY + config.TASK_HEIGHT + selectionPadding)
  selectionGraphics.lineTo(layout.startX + layout.radius, layout.topY + config.TASK_HEIGHT + selectionPadding)
  selectionGraphics.arc(layout.startX + layout.radius, layout.centerY, selectionRadius, Math.PI / 2, -Math.PI / 2, false)
