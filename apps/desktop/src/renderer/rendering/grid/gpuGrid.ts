...
function hexToRgb01(hex: number): { r: number; g: number; b: number } { const r = ((hex >> 16) & 0xff) / 255; const g = ((hex >> 8) & 0xff) / 255; const b = (hex & 0xff) / 255; return { r, g, b } }
export class GpuTimeGrid { container: Container; private quad: Graphics; private filter: Filter; constructor(app: Application) { this.container = new Container(); this.container.eventMode = 'none'; this.quad = new Graphics(); this.container.addChild(this.quad); const wgsl = `
...
fn mainFragment(@location(0) uv: vec2<f32>, @builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
  ...
  updateUniforms(input: { screenWidth: number; screenHeight: number; leftMarginPx: number; viewportXDays: number; dayWidthPx: number; minorStepDays: number; majorStepDays: number; minorColor: number; majorColor: number; minorAlpha: number; majorAlpha: number; minorLineWidthPx: number; majorLineWidthPx: number; scaleType: TimeScale; baseDow: number; weekendAlpha: number; globalAlpha: number; bandAlpha?: number }): void {
...
export function createGpuTimeGrid(app: Application): GpuTimeGrid { return new GpuTimeGrid(app) }
