...
import { UI_CONFIG } from '@cadence/config'

const MIN_WIDTH = UI_CONFIG.SIDEBAR.MIN_WIDTH
const MAX_WIDTH = UI_CONFIG.SIDEBAR.MAX_WIDTH
const DEFAULT_WIDTH = UI_CONFIG.SIDEBAR.DEFAULT_WIDTH
...
export function useResizableSidebar() {
  ...
  const clampSidebarWidth = useCallback(
    (w: number) => Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, w)),
    []
  )
  const onMouseMoveResize = useCallback(
    (e: MouseEvent) => {
      ...
      try {
        localStorage.setItem('cadence.sidebar.width', String(next))
      } catch { }
    },
    [clampSidebarWidth]
  )
  ...
  const resetSidebarWidth = useCallback(() => {
    ...
    try {
      localStorage.setItem('cadence.sidebar.width', String(DEFAULT_WIDTH))
    } catch { }
  }, [])
