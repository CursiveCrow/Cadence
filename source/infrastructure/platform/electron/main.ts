...
// ---------------- IPC handlers ----------------
ipcMain.handle(IPC_CHANNELS.dialogOpenFile, async (_event: unknown, options: unknown) => {
...
ipcMain.handle(IPC_CHANNELS.dialogSaveFile, async (_event: unknown, options: unknown) => {
...
ipcMain.handle(IPC_CHANNELS.fsReadFile, async (_event: unknown, filePath: unknown) => {
...
ipcMain.handle(IPC_CHANNELS.fsWriteFile, async (_event: unknown, filePath: unknown, content: ArrayBuffer | Buffer | Uint8Array) => {
  if (!isAllowedPath(filePath)) {
    throw new Error('Access denied: path not allowed')
  }
...
ipcMain.handle(IPC_CHANNELS.dialogMessageBox, async (_event: unknown, options: unknown) => {
